	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('index','view','create','update','admin','delete','dynamicPeople','dynamicPeopleCheck'),
				'users'=>array('admin'),
			),
        ...
        public function actionDynamicPeople($id = null)
       {
          try{
            ...
            if(is_null($entity) )
                ...
                //throw Exception("Hey that isn't a valid entity id");

            $entitypeople = $entity->people();
            // not sure about entityId
            //$data=Entity::model()->with('people')->findAll('parentId=:parentId',
              //      array(':parentId'=> $id));
//            $data=Entity::model()->with('people')->findByPk(1);
//           $data=Person::model()->findAll('entityId=:entityId', 
//                        array(':entityId'=>(int) $_POST['entities']));

           /*$data=CHtml::listData(
                   Person::model()->with('entities')->findAll(),
                   'idPerson',
                   'firstName');*/
//           foreach($data as $value=>$name)
           if($entitypeople) {
// devs idea             echo  CHtml::activeCheckBoxList($)
           foreach($entitypeople as $person)
           {
               //echo CHtml::listData($person, 'idPerson', 'firstName');
               ...
               //echo CHtml::listData($entitypeople, 'idPerson', 'firstName');
                ...
                foreach($entitypeople as $person)
                    ...
                    echo CHtml::tag('input',
                            array('type'=>'checkbox','checked'=>'checked','value'=>$person->idPerson),
